...
public final class FileUtils
{
	...
	/**
	 * Returns the absolute resource name for the specified class and relative
	 * resource name.
	 * 
	 * @param cls          the class whose package contains the resource.
	 * @param relativeName the relative resource name for which the absolute name is
	 *                     required.
	...
	public static BufferedReader loadResource(Class<?> cls, String relativeName)
	{
		...
		if (is == null) {
			throw new NullPointerException(
					"Class: " + cls.getSimpleName() + ", Resource: " + absoluteLoc);
		} else {
	 ...
	 */
	@SafeVarargs
	public static Vec<String> cacheResource(Class<?> cls, String relativeName,
			Predicate<? super String>... filters)
	{
	 ...
	 */
	public static Vec<String> cacheResource(Class<?> cls, String relativeName,
			Collection<Predicate<? super String>> filters)
	{
		try (BufferedReader reader = loadResource(cls, relativeName)) {
			return Vec.copy(reader.lines().map(String::trim).filter(x -> !x.isEmpty())
