...
public final class PinnedPieces
{
	...
	/**
	 * Given a board state this function calculates a List of pinned active pieces. That is those pieces
	 * who have extra constraints on their legal moves in order that the active side cannot move into
	 * check.
	 *
	...
	public static PinnedPieceCollection in(final BoardState state)
	{
		...
		return Iterate.over(ChessPieces.pinnersOn(passive))
				/* Map piece to a Flow of its locations at which it would attack king on empty board
				 * and flatten (we no longer need piece info).
				 */
