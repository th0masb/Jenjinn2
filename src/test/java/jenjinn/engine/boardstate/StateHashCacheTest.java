...
class StateHashCacheTest
{
	...
	void testIncrementClockValue()
	{
		...
		final StateHashCache start = new StateHashCache(IterRange.to(cacheSize).mapToLong(x -> x).toArray(), 0);

		for (int i = 0; i < 2*cacheSize; i++)
		{
			final long expectedDiscard = i < cacheSize - 1 ? i + 1 : 0;
			Assertions.assertEquals(expectedDiscard, start.incrementHalfMoveCount());
			...
			Assertions.assertEquals(new StateHashCache(expectedCache, i + 1), start);
		}
	}
	...
	{
		final int cacheSize = StateHashCache.CACHE_SIZE;
		final StateHashCache start = new StateHashCache(IterRange.to(cacheSize).mapToLong(x -> -1L).toArray(), cacheSize);
		...
		for (int i = 0; i < replacementHashes.length; i++)
		{
			...
			Assertions.assertEquals(new StateHashCache(expectedCache, cacheSize - (i + 1)), start, str(i));
		}
	}
}
